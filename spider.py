...
def closed(self, response):
    # grab the --output arg
    try:
        outputfile = filter(re.compile("--output=").match, sys.argv)[0].split(
            "--output="
        )[1]
    except:
...
def parse(self, response):
    ...
    for i in range(0, len(urls)):
        yield scrapy.Request(response.urljoin(urls[i]), callback=self.urlparse)
...
def urlparse(self, response):
    ...
    if match:
        ...
        # JSON output data
        yield {"headline": article.title, "url": url, "text": article.cleaned_text}
